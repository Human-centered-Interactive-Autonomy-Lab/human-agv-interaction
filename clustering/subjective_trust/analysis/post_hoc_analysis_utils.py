...
def run_pre_post_tests_auto(
    ...
    *,
    ...
    """
    print("\n[DEBUG] Starting run_pre_post_tests_auto...")
    print(f"[DEBUG] Input df shape: {df.shape}")
    print(f"[DEBUG] Cluster df shape: {cluster_df.shape}")
    ...
    def _one_row_per_group(d: pd.DataFrame, group_keys: list[str], cols: list[str]) -> pd.DataFrame:
        ...
        collapsed = tmp.groupby(group_keys, as_index=False).apply(_collapse)
    ...
    def _welch_df(x: np.ndarray, y: np.ndarray) -> float:
        """Degrees of freedom for the Welch t-test."""
        x = np.asarray(x, dtype=float)
        y = np.asarray(y, dtype=float)
        nx, ny = len(x), len(y)
        if nx < 2 or ny < 2:
    ...
    print(f"[DEBUG] Pre-survey columns found: {len(have_pre)} out of {len(pre_survey_cols)}")
    if have_pre:
        print(f"[DEBUG] Processing pre-survey data...")
        ...
        print(f"[DEBUG] Pre-survey one-row-per-group shape: {pre_one.shape}")
        _run_for_dataset(pre_one, have_pre, "Pre", out_rows)
        print(f"[DEBUG] Completed pre-survey tests, total rows: {len(out_rows)}")

    have_post = [c for c in post_survey_cols if c in df.columns]
    print(f"[DEBUG] Post-survey columns found: {len(have_post)} out of {len(post_survey_cols)}")
    if have_post:
        print(f"[DEBUG] Processing post-survey data...")
        ...
        for order in ["First", "Second"]:
            print(f"[DEBUG] Processing Post_{order} survey data...")
            post_cond = post_one[post_one["Survey_Order"] == order].copy()
            print(f"[DEBUG] Post_{order} subset shape: {post_cond.shape}")
            _run_for_dataset(post_cond, have_post, f"Post_{order}", out_rows)
            print(f"[DEBUG] Completed Post_{order} tests, total rows: {len(out_rows)}")

    out = pd.DataFrame(out_rows)
    print(f"[DEBUG] Created results DataFrame with shape: {out.shape}")
    ...
    print("[DEBUG] run_pre_post_tests_auto completed!\n")
    return out
